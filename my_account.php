					...
					<div id="products_box" style="background-image: url(images/bg3.jpg)">
						<?php 
							...
							while($row_user=mysqli_fetch_array($run_user)){
								...
								$tran_id=$row_user['tran_id'];

								$sel_tran_name="SELECT tran_name FROM trainer WHERE tran_id=$tran_id";

								$tran_name=mysqli_query($con,$sel_tran_name);

								$tran_name = $tran_name->fetch_array()[0] ?? '';
