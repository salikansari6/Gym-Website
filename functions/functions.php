<?php 
	...
	function getExercise($id){
		global $db;
		$get_exer="SELECT * FROM exercises where user_id=$id";
	...
	function get_Day_Exercises($id){
		global $db;
		if(isset($_GET['day'])){

				$day_id=$_GET['day'];

				$get_exer="SELECT * FROM exercises WHERE day_id='$day_id' AND user_id=$id";
				$run_exer=mysqli_query($db, $get_exer);
				$count= mysqli_num_rows($run_exer);
				if($count==0){
					echo "<h3 style='color:white'>"."No Workout Available or This Day."."</h3>";
				}
				while($row_exer=mysqli_fetch_array($run_exer)){
					$exer_id= $row_exer['exer_id'];
					$exer_name= $row_exer['exer_name'];
	...
	function get_All_Exercises($id){
		global $db;
		if(!isset($_GET['day'])){
			if(!isset($_GET['exer'])){

				$get_all_exer="SELECT * FROM exercises WHERE user_id=$id ORDER By rand() LIMIT 0,5";
